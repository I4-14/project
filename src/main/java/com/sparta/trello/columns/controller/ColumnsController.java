...
public class ColumnsController {
    ...
    public ResponseEntity<ApiResponse<ColumnsResponseDto>> createColumns(@PathVariable("id") Long id, @RequestBody ColumnsRequestDto requestDto) {
        ColumnsResponseDto responseDto = columnsServices.createColumns(id, requestDto);
        ApiResponse<ColumnsResponseDto> response = new ApiResponse<>("컬럼 생성성공", HttpStatus.CREATED.value() + "", responseDto);
        return new ResponseEntity<>(response, HttpStatus.CREATED);
    }
    @GetMapping("/columns/{id}")
    public ResponseEntity<ApiResponse<ColumnsResponseDto>> getColumnsById(@PathVariable("id") Long id) {
        ColumnsResponseDto responseDto = columnsServices.getColumnsById(id);
        ApiResponse<ColumnsResponseDto> response = new ApiResponse<>("컬럼 조회성공", HttpStatus.OK.value() + "", responseDto);
        return new ResponseEntity<>(response, HttpStatus.OK);
    }
    @PutMapping("/columns/{id}")
    public ResponseEntity<ApiResponse<ColumnsResponseDto>> updateColumns(@PathVariable("id") Long id, @RequestBody ColumnsRequestDto requestDto) {
        ColumnsResponseDto responseDto = columnsServices.updateColumns(id, requestDto);
        ApiResponse<ColumnsResponseDto> response = new ApiResponse<>("컬럼 수정성공", HttpStatus.OK.value() + "", responseDto);
        return new ResponseEntity<>(response, HttpStatus.OK);
    }
    @DeleteMapping("/columns/{id}")
    public ResponseEntity<ApiResponse> deleteColumns(@PathVariable("id") Long id) {
        columnsServices.deleteColumns(id);
    ...
    public ResponseEntity<ApiResponse> changeOrderColumns(@PathVariable("id") Long id, @PathVariable("destinationId") Long destinationId) {
        columnsServices.changeOrderNum(id, destinationId);
